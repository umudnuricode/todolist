...
// });
document.querySelector(".arrow img").addEventListener("click", (e) => {
    if (e.target.src.includes("Frame(6).svg")) {
        e.target.src = "Frame(7).svg";
        const inputs = document.querySelectorAll(".maintext input");
        const items = Array.from(inputs).map(input => input.parentElement);
        ...
        const container = document.querySelector(".maintext");
        container.innerHTML = '';
        items.forEach(item => container.appendChild(item));
    } else {
        e.target.src = "Frame(6).svg";
        const inputs = document.querySelectorAll(".maintext input");
        const items = Array.from(inputs).map(input => input.parentElement);
...
function add() {
    ...
    if (mainInput.value !== '') {
        ...
        maintext.innerHTML += `
            <div class="input2">
                <input type="text" readonly value="${mainInput.value}">
                <img src="Frame(5).svg" alt="" onclick="remove(this)" onmouseenter="change1(this)" onmouseleave="change2(this)">
            </div>`;
        mainInput.value = '';
    }
    ...
    if (input3.length > 0) {
        const last = input3[input3.length - 1];
        last.style.display = "none";
        const val = last.querySelector("input").value;
        if (val !== '') {
            maintext.innerHTML += `
                <div class="input2">
...
document.querySelector(".arrow img").addEventListener("mouseenter", () => {
    ...
    if (img.src.includes("Frame(4).svg")) {
        img.src = "Frame(6).svg";
    } else {
        img.src = "Frame(7).svg";
    }
...
document.querySelector(".arrow img").addEventListener("mouseleave", () => {
    const img = document.querySelector(".arrow img");
    if (img.src.includes("Frame(7).svg")) {
        img.src = "Frame(3).svg";
    } else {
        img.src = "Frame(4).svg";
    }
...
document.querySelector(".input img").addEventListener("mouseenter", () => {
    document.querySelector(".input img").src = "Group70.svg";
});

document.querySelector(".input img").addEventListener("mouseleave", () => {
    document.querySelector(".input img").src = "Frame(5).svg";
});
